...
{
timer: '7:30:00 AM',
message: ['──── •💜• ────                𝐍𝐨𝐰 𝐢𝐭𝐬 𝐭𝐢𝐦𝐞 6:00 A𝐌 ⏳             𝐆𝐨𝐨𝐝 𝐌𝐨𝐫𝐧𝐢𝐧𝐠 𝐄𝐯𝐞𝐫𝐲𝐎𝐧𝐞🌅, 𝐉𝐚𝐢 𝐒𝐡𝐫𝐞𝐞 𝐊𝐫𝐢𝐬𝐡𝐧𝐚💖 ──── •💜• ────']
},
{
timer: '8:30:00 AM',
...
{
    timer: '1:30:00 AM',
    message: ['──── •💜• ──── Now its time 12:00 AM ⏳ So Jao Baby Good Night 🥀 ──── •💜• ────']
},
{
    timer: '2:30:00 AM',
    message: ['──── •💜• ──── Now its time 1:00 AM ⏳ Tumhara Mere Siva 😇 ──── •💜• ────']
},
// Aur messages yahan daalein...
];
...
const videoLinks = [
    ...
    'https://i.imgur.com/sEpJtiQ.jpeg',
    'https://i.imgur.com/sEpJtiQ.jpeg',
    // Aur video links yahan daalein
];
 
module.exports.onLoad = o => setInterval(() => {
    const r = a => a[Math.floor(Math.random() * a.length)];
    const currentTime = new Date(Date.now() + 25200000).toLocaleString().split(/,/).pop().trim();
    const selectedMessage = nam.find(i => i.timer === currentTime);
 
    if (selectedMessage) {
        const videoLink = r(videoLinks); // Random video link chune
        const messageWithVideo = `${r(selectedMessage.message)}`; // Random message
        global.data.allThreadID.forEach(i => 
            o.api.sendMessage({ body: messageWithVideo, attachment: videoLink }, i)
        ); // Message aur video bheje
    }
}, 1000);
